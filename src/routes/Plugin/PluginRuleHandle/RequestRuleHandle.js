...
export default class RequestRuleHandle extends Component {
  ...
  initListByType = (handle, dataType) => {
      ...
      [
        {
          ...
          fieldName: `${dataType}_type_0`,
          fieldValue: `add${dataTypeFirstUpper}s`,
        },
        {
          fieldLabel: getIntlContent(
            `SHENYU.PLUGIN.REQUEST.${dataTypeUpper}.KEY`,
          ),
          fieldName: `${dataType}_key_0`,
          fieldValue: null,
        },
        {
          fieldLabel: getIntlContent(
            `SHENYU.PLUGIN.REQUEST.${dataTypeUpper}.VALUE`,
          ),
          fieldName: `${dataType}_value_0`,
          fieldValue: null,
        },
      ],
    ];
    if (
      handle &&
      handle[dataType] &&
      ((handle[dataType][addFields] &&
        Object.keys(handle[dataType][addFields]).length > 0) ||
        (handle[dataType][replaceFieldKeys] &&
          Object.keys(handle[dataType][replaceFieldKeys]).length > 0) ||
        (handle[dataType][setFields] &&
          Object.keys(handle[dataType][setFields]).length > 0) ||
        (handle[dataType][removeFields] &&
          handle[dataType][removeFields].length > 0))
    ) {
      list = [];
      let index = 0;
      ...
      handle[dataType][addFields] &&
        Object.keys(handle[dataType][addFields]).length > 0 &&
        Object.keys(handle[dataType][addFields]).forEach((e) => {
          let v = handle[dataType][addFields][e];
          list.push([
            {
              fieldLabel: "OperateType",
              fieldName: `${dataType}_type_${index}`,
              fieldValue: `add${dataTypeFirstUpper}s`,
            },
            {
              fieldLabel: getIntlContent(
              ...
              fieldName: `${dataType}_key_${index}`,
              fieldValue: e,
            },
            {
              fieldLabel: getIntlContent(
              ...
              fieldName: `${dataType}_value_${index}`,
              fieldValue: v,
            },
          ]);
          index += 1;
      ...
      handle[dataType][replaceFieldKeys] &&
        Object.keys(handle[dataType][replaceFieldKeys]).length > 0 &&
        Object.keys(handle[dataType][replaceFieldKeys]).forEach((e) => {
          let v = handle[dataType][replaceFieldKeys][e];
          list.push([
            {
              fieldLabel: "OperateType",
              fieldName: `${dataType}_type_${index}`,
              fieldValue: `replace${dataTypeFirstUpper}Keys`,
            },
            {
              fieldLabel: getIntlContent(
                `SHENYU.PLUGIN.REQUEST.${dataTypeUpper}.OLD.KEY`,
              ),
              fieldName: `${dataType}_key_${index}`,
              fieldValue: e,
            },
            {
              fieldLabel: getIntlContent(
                `SHENYU.PLUGIN.REQUEST.${dataTypeUpper}.NEW.KEY`,
              ),
              fieldName: `${dataType}_value_${index}`,
              fieldValue: v,
            },
      ...
      handle[dataType][setFields] &&
        Object.keys(handle[dataType][setFields]).length > 0 &&
        Object.keys(handle[dataType][setFields]).forEach((e) => {
          let v = handle[dataType][setFields][e];
          list.push([
            {
              fieldLabel: "OperateType",
              fieldName: `${dataType}_type_${index}`,
              fieldValue: `set${dataTypeFirstUpper}s`,
            },
            {
              fieldLabel: getIntlContent(
                `SHENYU.PLUGIN.REQUEST.${dataTypeUpper}.KEY`,
      ...
      handle[dataType][removeFields] &&
        handle[dataType][removeFields].length > 0 &&
        handle[dataType][removeFields].forEach((e, i) => {
          if (i % 2 === 0) {
            removeKeys.push([]);
          }
          ...
          let dataItem = [
            {
              fieldLabel: "OperateType",
              fieldName: `${dataType}_type_${index}`,
              fieldValue: `remove${dataTypeFirstUpper}Keys`,
            },
            {
              fieldLabel: getIntlContent(
                `SHENYU.PLUGIN.REQUEST.${dataTypeUpper}.KEY`,
  ...
  handleAddRow = (type) => {
    ...
    list.push([
      {
        fieldLabel: "OperateType",
        fieldName: `${type}_type_${index}`,
        fieldValue: defaultFieldType,
      },
      {
        fieldLabel: getIntlContent(
          `SHENYU.PLUGIN.REQUEST.${type.toUpperCase()}.KEY`,
        ),
        fieldName: `${type}_key_${index}`,
        fieldValue: null,
      },
      {
        fieldLabel: getIntlContent(
          `SHENYU.PLUGIN.REQUEST.${type.toUpperCase()}.VALUE`,
        ),
        fieldName: `${type}_value_${index}`,
        fieldValue: null,
      },
    ]);
    this.setState({
  ...
  handleTypeChange = (val, type, rowIndex) => {
    ...
    if (val.startsWith("replace")) {
      list[rowIndex][1].fieldLabel = getIntlContent(
        `SHENYU.PLUGIN.REQUEST.${type.toUpperCase()}.OLD.KEY`,
      );
      list[rowIndex][2].fieldLabel = getIntlContent(
        `SHENYU.PLUGIN.REQUEST.${type.toUpperCase()}.NEW.KEY`,
      );
                ...
                return (
                  <Row gutter={24} key={rowIndex} style={rowStyle}>
                    ...
                    <Col span={6}>
                      ...
                      {rowIndex === 0 && (
                        <Button
                          onClick={() => this.handleAddRow("header")}
                          type="primary"
                        >
                          {getIntlContent("SHENYU.COMMON.ADD")}
                        </Button>
                ...
                return (
                  <Row gutter={24} key={rowIndex} style={rowStyle}>
                    ...
                    <Col span={6}>
                      ...
                      {rowIndex === 0 && (
                        <Button
                          onClick={() => this.handleAddRow("cookie")}
                          type="primary"
                        >
                          {getIntlContent("SHENYU.COMMON.ADD")}
                        </Button>
