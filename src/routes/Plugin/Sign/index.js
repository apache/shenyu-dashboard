...
export default class Sign extends Component {
  ...
  getAllSelectors = (page, plugins) => {
    const { dispatch } = this.props;
    const pluginId = this.getPluginId(plugins, "sign");
    dispatch({
      type: "sign/fetchSelector",
      payload: {
        currentPage: page,
        pageSize: 12,
        pluginId
  ...
  asyncClick = () => {
    const { dispatch, plugins } = this.props;
    const id = this.getPluginId(plugins, "sign");
    dispatch({
