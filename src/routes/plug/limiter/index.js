...
@connect(({ limiter, global, loading }) => ({
  ...global,
  ...limiter,
...
export default class Limiter extends Component {
  ...
  getAllSelectors = (page, plugins) => {
    const { dispatch } = this.props;
    const pluginId = this.getPluginId(plugins, "rate_limiter");
    dispatch({
      type: "limiter/fetchSelector",
      payload: {
        currentPage: page,
        pageSize: 12,
        pluginId
  ...
  rowClick = record => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: "limiter/saveCurrentSelector",
      payload: {
  ...
  asyncClick = () => {
    const { dispatch, plugins } = this.props;
    const id = this.getPluginId(plugins, "rate_limiter");
    dispatch({
