...
class AddModal extends Component {
  ...
  constructor(props) {
    super(props);
    const labelList = Object.entries(props.labels || {}).map(([name, value], key) => ({ key, name, value }))
    if (!labelList.length) {
  ...
  sendTest = (e) => {
    ...
    form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        handleSendTest({ ...values, id, labels: Object.fromEntries(this.state.labelList.map(({ name, value }) => [name, value])) });
      }
    });
  }
          ...
          return (
            ...
            >
              {getFieldDecorator('email', {
                rules: [{ required: true, message: getIntlContent("SHENYU.SYSTEM.ALERT.EMAIL.INPUT") }],
                initialValue: email,
              })(
                <Input allowClear placeholder={getIntlContent("SHENYU.SYSTEM.ALERT.EMAIL.INPUT")} />
              )}
            </FormItem>
          )
        case "5":
          return (
            ...
            >
              {getFieldDecorator('accessToken', {
                rules: [{ required: true, message: getIntlContent("SHENYU.SYSTEM.ALERT.ACCESS_TOKEN.INPUT") }],
                initialValue: accessToken,
              })(
                <Input allowClear placeholder={getIntlContent("SHENYU.SYSTEM.ALERT.ACCESS_TOKEN.INPUT")} />
              )}
            </FormItem>
          )
        default: return null
        ...
        <Form onSubmit={this.handleSubmit} className="login-form">
          ...
          >
            {getFieldDecorator('name', {
              rules: [{ required: true, message: getIntlContent("SHENYU.SYSTEM.ALERT.NAME.INPUT") }],
              initialValue: name,
            })(
              <Input allowClear placeholder={getIntlContent("SHENYU.SYSTEM.ALERT.NAME.INPUT")} />
            )}
          </FormItem>
          <FormItem
          ...
          >
            {getFieldDecorator('type', {
              rules: [{ required: true, message: getIntlContent("SHENYU.SYSTEM.ALERT.TYPE.SELECT") }],
              ...
              >
                {Object.entries(Type).map(([k, v]) => <Option value={k} key={k}>{getIntlContent(v)}</Option>)}
              </Select>
            )}
          </FormItem>
          {renderConfiguration()}
          ...
          >
            {getFieldDecorator('matchAll', {
              rules: [{ required: true, message: getIntlContent("SHENYU.SYSTEM.ALERT.MATCH_ALL.SELECT") }],
          ...
          </FormItem>
          {
            ...
            <>
              ...
              <Select
                mode="tags"
                placeholder={getIntlContent("SHENYU.SYSTEM.ALERT.LEVELS.SELECT")}
                style={{ width: '100%' }}
                onChange={v =>
                  form.setFieldsValue({ levels: v.sort((l, r) => parseInt(l, 10) - parseInt(r, 10)) })
                }
              >
                <Option value="0">{getIntlContent("SHENYU.SYSTEM.ALERT.LEVELS.HIGH")}</Option>
                <Option value="1">{getIntlContent("SHENYU.SYSTEM.ALERT.LEVELS.MIDDLE")}</Option>
