...
export default class Home extends Component {
  ...
  getEventLogTitle = (log) => {
    ...
    return (
      <div style={{ display: "inline" }}>
        ...
        {/* eslint-disable-next-line prettier/prettier */}Ï
  ...
  showEventLogDetail = (e, log) => {
    ...
    Modal.info({
      ...
      width: 500,
      content: (
        <div style={{ maxHeight: 400, overflowY: "auto" }}>{log.context}</div>
      ),
      onOk() {},
    });
  };
  ...
  render() {
    ...
    const pluginSteps = this.state.activePluginSnapshot.map((p, index) => {
      const content = (
        <div>
          <p>
            <Tag color={colors[(p.role.length + index) % colors.length]}>
              {p.role}
            </Tag>
          </p>
          <p>
            the plugin has handle is :{" "}
            <span style={contextStyle}>{p.handleCount}</span>
          </p>
          <p>
            the plugin has selector is :{" "}
            <span style={contextStyle}>{p.selectorCount} </span>
          </p>
          <hr />
      ...
      const title = (
        ...
        >
          <Tag
            color="geekblue"
            onClick={this.pluginOnClick.bind(this, p)}
      ...
      const description = (
        <span>
          handle is <span style={contextStyle}>{p.handleCount}</span> selector
          is <span style={contextStyle}>{p.selectorCount} </span>
        </span>
      );
      return <Step title={title} key={index} description={description} />;
    });
    const activeLogItems = this.state.activeLog.map((log, index) => {
      const type = log.operationType.startsWith("CREATE")
        ? "success"
        : log.operationType.startsWith("DELETE")
          ? "warning"
          : "info";
      return (
        <Timeline.Item color="#e8e8e8" label={index} key={index}>
          <div
            ...
            onClick={(e) => this.showEventLogDetail(e, log)}
          >
            <Alert
      ...
      <div>
        ...
        <div className={styles.processContent}>
          <Steps current={1}>
            ...
            <Step
              title="Shenyu-Gateway BootStrap Proxy"
              subTitle="plugin list"
          ...
          <Row gutter={16} className={styles.row}>
            <Col span={12}>
              <Card
                title="Activity plugin list"
                bordered={false}
                className={styles.card}
              >
                <Steps
                  size="small"
                  current={this.state.activePluginSnapshot.length}
                  direction="vertical"
                >
                  {pluginSteps}
                </Steps>
            ...
            <Col span={12}>
              <Card title="Event log" bordered={false} className={styles.card}>
                <Timeline>{activeLogItems}</Timeline>
                <Divider />
              </Card>
            </Col>
          </Row>
